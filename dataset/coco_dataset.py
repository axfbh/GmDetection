...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        target = coco_to_boxes(img, target)
        img = np.array(img)
        ...
        target = self.convert_albumen_to_coco_fmt(target, sample)
    ...
    def convert_albumen_to_coco_fmt(self, target, sample):
        ...
        target['labels'] = torch.tensor(sample['classes'], dtype=torch.long)
...
def build_coco_dataset(img_folder, ann_file, imgsz, mode='train', return_masks=False):
    return CocoDetection(img_folder,
                         ann_file,
                         imgsz=imgsz,
                         transforms=None if mode == 'train' else None)
