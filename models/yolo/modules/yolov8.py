...
from backbone.cspdarknet import CSPDarknetV8, CBM, C2f
from backbone.utils import _cspdarknet_extractor
from nn import YoloHeadV8


# from utils.modules.detection.yolo.model import YoloModel
# from utils.loss.yolo_loss import YoloLossV8


class YoloV8(YoloModel):
    def __init__(self, num_classes, scales, *args, **kwargs):
        super(YoloV8, self).__init__(*args, **kwargs)

        width_multiple = {'n': 0.25, 's': 0.50, 'm': 0.75, 'l': 1.0, 'x': 1.25}[scales]
        depth_multiple = {'n': 0.33, 's': 0.33, 'm': 0.67, 'l': 1.0, 'x': 1.0}[scales]
        deep_mul = {'n': 1.00, 's': 1.00, 'm': 0.75, 'l': 0.50, 'x': 0.50}[scales]
        ...
        self.head = YoloHeadV8([base_channels * 4, base_channels * 8, base_channels * 16], num_classes=num_classes)
